...
{
    public class QuestionBlockCollision : ICommand
    {
        Direction CollisionSide;
        public BlockCollisionManager BlockCollisionManager;
        public QuestionBlockCollision(object nRef) : base(nRef)
        {
            CollisionSide = new Direction();
        }
        public override void Execute()
        {
            CollisionSide = CollisionDetector.getCD().DetectColDirection(BlockCollisionManager.QBlocK, BlockCollisionManager.mario);
            if (BlockCollisionManager.mario.GetType() == typeof(BigMario) || BlockCollisionManager.mario.GetType() == typeof(FireMario) || BlockCollisionManager.mario.GetType() == typeof(StarMario) || BlockCollisionManager.mario.GetType() == typeof(SmallMario))
            {
                ...
                {
                    BlockCollisionManager.QBlocK.State.ChangeState((int)State.BUMPING); // Not sure whether it's the right thing should happen
